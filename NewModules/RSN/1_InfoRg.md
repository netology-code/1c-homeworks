...
Помните, что повторяющийся код, лучше выносить в общие модули. Т.к. аналогичная обработка заполнения была реализована и для предыдущего регистра, можно прописать код в общем модуле и параметром в него передавать Набор записей (ЭтотОбъект)
